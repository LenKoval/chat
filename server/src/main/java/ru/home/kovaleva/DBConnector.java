...
public class DBConnector {
    // отделить коннект и дисконнект с базой
    // сделать класс который отдельно работает с юзерами
    private Connection connection;
    private User user;
    private static final String DATABASE_URL = "jdbc:postgresql://localhost:5432/postgres"; // убрать в проперти
    ...
    public List<User> selectUsers() {
        ...
        try (PreparedStatement ps = connection.prepareStatement(SELECT_USERS)) { // все запросы переделать при старте и добавить дисконнект
            ...
            throw new RuntimeException(e); // сделать анчект для бд
        }
