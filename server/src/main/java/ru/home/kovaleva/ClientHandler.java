...
public class ClientHandler {
    ...
    private String username; // добавить юзера
    ...
    public ClientHandler(Socket socket, Server server) throws IOException {
        ...
        if (userCount < maxUserCount) username = "ru.home.kovaleva.User" + userCount++;

        logger.info("Client " + username + " is connected.");
        ...
        while (true) {
            ...
            if (message.startsWith("/")) {
                if (message.equals("/exit")) {
                    ...
                    if (server.getAuthenticationProvider().checkAccess(info)) { // этого быть не должно прверяется юзер
                    ...
                    String info = in.readUTF();
                    if (server.getAuthenticationProvider().checkAccess(info)) {
                        server.stop(); // опрокинуть клиентхандлер и проверка не нужна
                    } else {
                        sendMessage("У вас нет прав для операции.");
                    }
                }
        ...
        while (!isAuthenticated) {
            ...
            switch (command) {
                case "/auth": {
                    ...
                    if (username == null || username.isBlank()) {
                    ...
                    } else { // убрать else добавить continue
                        ...
                        isAuthenticated = true; //если user !null заменить
                    }
                    break;
                }
                case "/register": {
