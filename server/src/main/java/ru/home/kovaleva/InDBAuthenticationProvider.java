...
public class InDBAuthenticationProvider implements AuthenticationProvider {
    private final List<User> users; // список убрать
    ...
    @Override
    public String getUsernameByLoginAndPassword(String login, String password) { // отдать бд
        for (User user : users) {
    ...
    public synchronized String[] changeUsername(String message) { // перенести проверку на уровне бд
    ...
    @Override
    public synchronized void changeBanUser(String message) { // сделать бан по времени
